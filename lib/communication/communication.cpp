...
void readContinuity(void *pvParameters) {
    ...
    for (;;) {
        ...
        // vTaskDelay(30 / portTICK_PERIOD_MS);  // Nhường CPU
    }
}
...
void receiveUART() {
    if (Port.available() > 0) {
        ...
        if(package.data[0]==0x42&&package.data[0]==0x44){
            package.data[0]-=1;
        ...
        // ngoai le
        else if(package.data[0]==0x57){
            digitalWrite(PIN_WARN,1);
        }
        else if(package.data[0]==0x58){
            digitalWrite(PIN_WARN,0);
        }
        else{
